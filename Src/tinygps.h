...
// #define _GPS_NO_STATS
 ...
 // static const float GPS_INVALID_F_ANGLE, GPS_INVALID_F_ALTITUDE, GPS_INVALID_F_SPEED;
  ...
  extern  long altitude(void);

  // course in last full GPRMC sentence in 100th of a degree
  extern  unsigned long course(void);

  // speed in last full GPRMC sentence in 100ths of a knot
  extern  unsigned long speed(void);

  // satellites used in last full GPGGA sentence
  extern  unsigned short satellites(void);

  // horizontal dilution of precision in 100ths
  extern  unsigned long hdop(void);
  ...
  extern float f_altitude(void);
  extern float f_course(void);
  extern float f_speed_knots(void);
  extern float f_speed_mph(void);
  extern float f_speed_mps(void);
  extern float f_speed_kmph(void);
